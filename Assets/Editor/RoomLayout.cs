                                   ...
                                   List<RoomBox> reserved = null)
    {
...

        {
            foreach (int k in stageNodes)
            {
                ...
                // ⛔ ПРИЦЕЛЬНАЯ ПОСАДКА ПОСЛЕДНЕГО ЗВЕНА ПРОБОВАЛАСЬ ДВАЖДЫ И ОТВЕРГНУТА (2026-09-11).
                //
                // Замысел. Звено, которому велено коснуться маршрута (nextToAny), ищет место теми же
                // случайными бросками, что и все: у боковой посадки случаен вертикальный сдвиг (13
                // значений), у вертикальной — боковой. Нужное значение при этом РОВНО ОДНО на каждую
                // комнату маршрута. Казалось очевидным: надо не бросать, а вычислять сдвиг из
                // положения комнат маршрута.
                //
                // 🐞 Заход первый — прицел по ОДНОЙ оси: клапанов 7 против 9, чистых 6 из 10 вместо 8.
                // 🐞 Заход второй — прицел сразу по стороне, зазору и сдвигу, то есть по обеим осям
                //    (в створ гарантированно): клапанов 8, чистых 6 из 10, поставка 19.30 против 20.10.
                //
                // ⚠️ ВЫВОД, КОТОРЫЙ СТОИЛ ДВУХ ЗАХОДОВ: раскладка промахивается не от слепоты.
                // Прицельная посадка ПОПАДАЕТ в створ — и оказывается хуже для всего остального:
                // звено притирается к трассе и отбирает место у следующих этапов. Случайный перебор
                // хуже замыкает петли, но лучше раскладывает уровень целиком, и второе важнее.
                // Чинить надо не выбор позиции последнего звена, а ФОРМУ ветки целиком — чтобы она
                // подходила к маршруту заранее, а не притиралась к нему на последнем шаге.
                // ⭐ ЗВЕНУ, КОТОРОЕ ТЯНЕТСЯ К ЦЕЛИ, ДАЁМ ГУЩЕ ВЫБОРКУ.
                // Притяжение (pullTo) перебирает ВСЕ броски и берёт ближайший к цели — значит
                // качество посадки упирается прямо в их число, а их двенадцать на всех.
                // ⚠️ Это НЕ прицел (он пробовался дважды и оба раза вышел хуже, см. ниже): правило
                // выбора то же самое, гуще только выборка. Лишние броски стоят только времени и
                // только на тех звеньях, которым и правда есть куда целиться.
                bool aiming = req[k].pullTo >= 0
                           || (req[k].nextToAny != null && req[k].nextToAny.Count > 0);
                int shots = aiming ? Shots * 4 : Shots;
