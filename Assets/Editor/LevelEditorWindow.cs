...
public class LevelEditorWindow : EditorWindow
{
    ...
    /// <summary>
    /// ⭐ СКОЛЬКО КОМНАТ СТРОИТЬ СВОБОДНОМУ ПУТИ — своя ручка, а не «сетка ↔×↕ поделить на три».
    ///
    /// ⚠️ Развёл специально: `_mazeW/_mazeH` делили ДВА разных генератора с разным смыслом. Свободный
    /// путь брал `W*H/3`, а старый лабиринт клампит их до 12 и выводит размер сетки из рецепта. Подняв
    /// общее поле, я молча поменял бы и второй генератор — а его никто не просил трогать.
    ///
    /// Значение по умолчанию поднято 2026-09-10 с 8 (это и давало прежнее 5×5) до 56: уровень растёт
    /// с 22 комнат и 68×34 до 54 комнат и 105×49. Раскидистость для этой игры желаема, а не терпима
    /// (решение игрока), и после починки сходимости раскладки она почти ничего не стоит — время
    /// приёмки держится на числе ГРУПП, а не на площади. Замер на умолчаниях панели: 16 сидов из 16
    /// собрались и все чисты, 165 мс на уровень.
    /// Проверенный потолок — 110 заказанных комнат (12/12 чистых, габарит 152×58).
    /// </summary>
    [SerializeField] private int _freeRooms = 56;
    ...
    /// </summary>
    /// <summary>
    /// Потолок по числу ГРУПП на уровень. ⚠️ Это ограничение ЦЕНЫ ПРОВЕРКИ, а не дизайна: поиск идёт
    /// по 2^групп состояний, и приёмка гоняет его ещё раз на каждую группу.
    ///
    /// ⭐ Поднят 2026-09-10 с 4 до 6. Четвёрка писалась, когда приёмка стоила ~1 с при четырёх
    /// группах; после перевода модели на битовую сетку и переиспользования модели она стоит ~0.2 с,
    ...
    private void DrawSchemeImport()
    {
        ...
        EditorGUIUtility.labelWidth = lwPrev;
        {
            ...
            _freeRooms = EditorGUILayout.IntSlider(new GUIContent("Комнат в уровне",
                ...
                + "поэтому уровень выходит чуть меньше заказа. Проверено до 110 (габарит 152×58)."),
                _freeRooms, 6, 160);
    ...
    private string GenerateFreeScheme()
    {
        ...
        // ⭐ ПОТОЛОК ПОДНЯТ 2026-09-10 С 40 ДО 160, и размер теперь берётся из СВОЕЙ ручки
        // (см. _freeRooms). Сорок стояли потому, что дальше раскладка не сходилась — а не потому,
        // что большой уровень плох (игрок: «раскидистые уровни это не минус, а плюс»). Причину нашли
        // и убрали: комнаты-доборы клались одной атомарной пачкой, и провал любой ронял уровень
        // целиком (см. RoomReq.optional). Замер после починки: 45 комнат 12/12, 60 — 12/12,
        // 80 — 12/12, 110 — 12/12, и все чисты по приёмке.
        int wantRooms = Mathf.Clamp(_freeRooms, 6, 160);
