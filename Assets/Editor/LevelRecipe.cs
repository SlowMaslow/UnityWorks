...
public enum PuzzleElement
{
    ...
    Vault,
    /// <summary>Сокровищница на таймере: три полки одной группы, лут слоями, дно из шипов.
    /// Разобрана из группы C ручного Level_09.</summary>
    LootPit,
    /// <summary>Лестница на таймере: короткие полки вразбежку, кнопка внизу.
    /// Разобрана из группы D ручного Level_09.</summary>
    Ladder
}
...
public struct SpaceNeed
{
    ...
    /// <summary>
    /// ⭐⭐ КАРТИНКА ЖИВЁТ В РОДИТЕЛЬСКОЙ КОМНАТЕ, А НЕ В КОМНАТЕ ЭЛЕМЕНТА.
    ///
    /// По умолчанию трафарет штампуется в комнату самого элемента (ребёнка ребра) — так устроена
    /// камера. Но подъёму это не подходит: у вертикального ребра родитель СНИЗУ, игрок приходит
    /// оттуда, и лестница обязана стоять там же, иначе ступени окажутся уже ЗА люком.
    /// 🐞 Замер, из-за которого флаг появился: лестница штамповалась в верхнюю комнату, и
    /// непроходимыми становились 9 уровней из 10 — при достижимом финише, собранных ключах и
    /// нулевом замуровывании. Всё за лестницей было просто не достать.
    /// ⚠️ Флаг переносит И требования к размеру: комнату ровно под картинку резервирует тот же,
    /// в кого она штампуется. Иначе размер закрепится за одной комнатой, а картинка ляжет в другую.
    /// </summary>
    public bool patternInParent;
}
...
public static class PuzzleVocabulary
{
    public static SpaceNeed Need(PuzzleElement e)
    {
        switch (e)
        {
            ...
            // ⭐ СОКРОВИЩНИЦА устроена как камера: трасса поверху, всё остальное роется в толще.
            // Отличие в содержимом, а не в посадке, поэтому и требования те же — сквозной коридор
            // ровно под ширину картинки и подвал под ней.
            case PuzzleElement.LootPit:
                return new SpaceNeed { rooms = 2,
                                       minWidth  = StencilLibrary.LootPit().Width - 2,
                                       maxWidth  = StencilLibrary.LootPit().Width - 2,
                                       minHeight = 3, maxHeight = 3,
                                       horizontalEdge = true, throughCorridor = true,
                                       // ⚠️⚠️ ФОРМУЛА ТА ЖЕ, ЧТО У КАМЕРЫ: Height − 6. Пять верхних
                                       // рядов — потолок, воздух, трасса и её пол; последний ряд —
                                       // требование камня под дном. Остальное и есть подвал.
                                       // 🐞 Я написал Height − 4, то есть заказал подвал на два ряда
                                       // глубже картинки. Раскладка резервировала лишнее место, и
                                       // ломалось ВСЁ, а не только сокровищница: замурованных клеток
                                       // 157 и 60 даже на тех сидах, где сокровищница не встала
                                       // вовсе, финиш недостижим, мёртвых групп до 21.
                                       cellarHeight = StencilLibrary.LootPit().Height - 6,
                                       // ⛔ holdsKey НЕ СТАВИМ: ключ в невозвратной яме делает уровень
                                       // непроходимым (замер — см. комментарий в StencilLibrary.LootPit).
                                       needsExit = StencilLibrary.LootPit().needsExit };

            // ⭐ ЛЕСТНИЦА живёт ВНУТРИ одной комнаты и ничего от соседей не требует: ни сквозного
            // коридора, ни подвала. Комната резервируется ровно под картинку, чтобы лестница не
            // оказалась декорацией в углу просторного зала.
            // ⛔ selfClimbing НЕ СТАВИМ, хотя рука тянется: подъём лестница и правда несёт сама, но
            // только пока идёт её окно. Комната выше общего потолка становится односторонней —
            // замер дал 11 непроходимых уровней из 20. Высота как у всех, ряды считает картинка.
            case PuzzleElement.Ladder:
                return new SpaceNeed { rooms = 2,
                                       minWidth  = StencilLibrary.TimedLadder().Width - 2,
                                       maxWidth  = StencilLibrary.TimedLadder().Width - 2,
                                       minHeight = StencilLibrary.TimedLadder().Height - 2,
                                       maxHeight = StencilLibrary.TimedLadder().Height - 2,
                                       verticalEdge = true, patternInParent = true };
    ...
    public static string Name(PuzzleElement e)
    {
        switch (e)
        {
            case PuzzleElement.LootPit:    return "сокровищница на таймере";
            case PuzzleElement.Ladder:     return "лестница на таймере";
...
public class LevelRecipe
{
                                                ...
                                                int maxGroups, bool allowExcursion)
    {
        ...
        int groups = 1, vaults = 0, lootPits = 0;
        ...
        while (rec.ElementCount < wantMechanisms && groups < maxGroups && guard++ < guardMax)
        {
            ...
            // ⛔ СОКРОВИЩНИЦА И ЛЕСТНИЦА (трафареты из ручного Level_09) ПОКА НЕ ЗАКАЗЫВАЮТСЯ.
            //
            // Обе картинки готовы и СТАВЯТСЯ: лестница 76 раз из 76 заказов, сокровищница 12 из 20.
            // Ломается ИНТЕГРАЦИЯ, а не форма — замер на 20 сидах: чистых 5 из 20 вместо 18, и
            // непроходимость не от замуровывания (финиш достижим, ключи собираются, замурованного
            // нет), а от односторонности.
            //
            // 🐞 Причина у лестницы найдена: она объявлена verticalEdge и штампуется в комнату-РЕБЁНКА,
            // то есть в ВЕРХНЮЮ. Игрок приходит снизу, ему надо подняться через люк — а ступени
            // нарисованы уже НАД люком, с той стороны, куда он ещё не попал. Всё за ней недостижимо.
            // У работающих вертикальных ворот зигзаг строится в НИЖНЕЙ комнате (VerticalGateModule
            // берёт s.roomId = родитель) — потому они и встают.
            //
            // ⚠️ Чинится переносом штампа в родительскую комнату, но это тянет за собой и требования
            // к размеру (сейчас они закреплены за комнатой ребёнка через maxWidth/maxHeight), и
            // выбор стороны. Отдельный заход. До него генератор не должен их заказывать.
            if (lootPits == 0) pick.Add(PuzzleElement.LootPit);
            // ⛔ ОГРАНИЧЕНИЕ «НЕ БОЛЬШЕ ДВУХ ЛЕСТНИЦ НА УРОВЕНЬ» ПРОБОВАЛ — СТАЛО ХУЖЕ (2026-09-12).
            // Рассуждение было по аналогии с камерой («она одна на уровень»): каждая лестница это
            // таймерный подъём на единственной связи между комнатами, и много таких в одно
            // прохождение не складывается. Замер опроверг: с потолком в две штуки чистых 2 из 10,
            // без потолка — 5 из 10. Значит дело не в числе лестниц, а в том, что рецепт при них
            // иначе набирает всё остальное.
            // ⛔ ЛЕСТНИЦА ПОКА НЕ ЗАКАЗЫВАЕТСЯ (2026-09-12). Посадку ей починили — она штампуется в
            // НИЖНЮЮ комнату и встаёт 77 раз из 79 заказов (было 1 чистый уровень из 10, стало 5).
            // Но она всё ещё стоит чистоты, и с сокровищницей складывается плохо:
            //   без обоих паттернов      — чистых 8 из 10;
            //   только сокровищница      — чистых 8 из 10 (бесплатна);
            //   только лестница          — чистых 5 из 10;
            //   обе вместе               — чистых 3 из 10, мёртвых монет 512 (целые области отрезаны).
            // ⚠️ Ограничение «не больше двух на уровень» пробовал — стало ХУЖЕ (2 из 10), значит
            // дело не в числе. Разбираться отдельно: почему таймерный подъём на вертикальном ребре
            // отрезает области, хотя сам встаёт и климб внутри него считается проходимым.
            //   pick.Add(PuzzleElement.Ladder);
            ...
            // ⭐ Сокровищница — тоже ПРОЛЁТ ТРАССЫ, а не ветка: игрок идёт по её коридору, и провал
            // в полу коридора его и гейтит. Ровно та же роль, что у камеры.
            if (e == PuzzleElement.LootPit)
            {
                main.elements.Add(PuzzleElement.LootPit);
                groups += 1; lootPits++;
                continue;
            }
