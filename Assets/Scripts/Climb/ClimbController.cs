...
public class ClimbController : MonoBehaviour
{
    ...
    private void LateUpdate()
    {
        ...
        if (!broken)
            for (int i = 0; i < 2; i++)
            {
                ...
                // СБРОС ДРЕЙФА: доснап (ниже) пишет в localPosition локтя/кисти, а Solve его НЕ
                // сбрасывает → оффсет НАКАПЛИВАЕТСЯ кадр за кадром и садится в равновесие, ЗАВИСЯЩЕЕ
                // от истории (на старте — «хороший» локоть, после разрыва+оживления — «излом»). Возвращаем
    ...
    public void Revive(Vector3 spawnPos)
    {
        ...
        // Форма рук после оживления теперь не требует спец-сброса: LateUpdate зовёт ik.ResetPose()
        // перед каждым Solve, поэтому поза детерминирована и совпадает со старт-спавном.
    }
